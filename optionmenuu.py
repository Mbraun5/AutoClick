...
class OptionMenu(tk.Frame):
    def __init__(self, *args, **kwargs):
        ...
        self.canvas = tk.Canvas(self, bd=0, yscrollcommand=self.scrollbar.set, width=226, height=104,
        ...
        self.config(height=200, width=226)
    ...
    def add_command(self, label=None, command=None):
        new_button = tk.Button(self.btnFrame, text=label, command=command, borderwidth=0, image=self.pixel,
    ...
    def post(self, padx, pady):
        if len(self.command_list) * 20 < 200:
            self.canvas.config(height=len(self.command_list) * 20)
        else:
